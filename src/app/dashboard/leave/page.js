...
export default function UnifiedLeaveDashboard() {
  ...
  return (
    <div style={{ maxWidth: '1200px' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem', flexWrap: 'wrap', gap: '1rem' }}>
        ...
        {!isHR && !isManager && (
          <button onClick={() => setShowApplyModal(true)} style={{
            display: 'flex', alignItems: 'center', gap: '0.375rem', padding: '0.625rem 1.25rem', background: '#7B5EA7', color: '#fff',
      ...
      {!isHR && !isManager && (
        <div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(180px, 1fr))', gap: '1rem', marginBottom: '2rem' }}>
            {balances.map(b => (
              <div key={b.id} style={{ background: '#fff', border: '1px solid #f0ece6', borderRadius: 12, padding: '1.25rem', boxShadow: '0 2px 8px rgba(0,0,0,0.04)' }}>
                <div style={{ fontSize: '0.75rem', color: '#9ca3af', fontWeight: 600, textTransform: 'uppercase', marginBottom: '0.5rem' }}>{b.leaveType?.name}</div>
                <div style={{ fontSize: '2rem', fontWeight: 800, color: b.balance <= 1 ? '#dc2626' : '#7B5EA7', lineHeight: 1 }}>{b.balance}</div>
                <div style={{ fontSize: '0.75rem', color: '#9ca3af', marginTop: '0.25rem' }}>days remaining</div>
              </div>
            ))}
          </div>
          ...
          <div style={{ background: '#fff', borderRadius: 16, border: '1px solid #f0ece6', overflow: 'hidden', boxShadow: '0 2px 8px rgba(0,0,0,0.04)' }}>
            <div style={{ padding: '1.25rem 1.5rem', borderBottom: '1px solid #f0ece6', fontWeight: 600 }}>My Leave History</div>
            {requests.length === 0 ? (
              <div style={{ padding: '3rem', textAlign: 'center', color: '#9ca3af' }}>No leave requests submitted yet.</div>
            ) : (
                  ...
                  {requests.map(req => {
                    ...
                    return (
                      <tr key={req.id} style={{ borderBottom: '1px solid #fafaf9' }}>
                        <td style={{ padding: '0.875rem 1rem', fontWeight: 500 }}>{req.leaveType?.name}</td>
                        <td style={{ padding: '0.875rem 1rem', color: '#4b5563' }}>{new Date(req.startDate).toLocaleDateString('en-IN', { day: '2-digit', month: 'short' })}</td>
                        <td style={{ padding: '0.875rem 1rem', color: '#4b5563' }}>{new Date(req.endDate).toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: 'numeric' })}</td>
                        <td style={{ padding: '0.875rem 1rem', fontWeight: 600 }}>{days}</td>
      ...
      {/* ==================== 2. MANAGER & HR APPROVAL VIEW ==================== */}
      {(isHR || isManager) && activeHRView === 'approvals' && (
        <>
          {/* Filters */}
