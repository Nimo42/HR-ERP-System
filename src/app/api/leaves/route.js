...
export async function GET(request) {
  try {
    ...
    // HR Manager and IT Owner see all
...
export async function POST(request) {
  try {
    ...
    if (!leaveType.isEmergency && (!balance || balance.balance < requestedDays)) {
      return NextResponse.json({ message: `Insufficient leave balance. You have ${balance?.balance ?? 0} days remaining.` }, { status: 400 });
    }
        ...
        html: `
          <div style="font-family: sans-serif; max-width: 560px; margin: 0 auto;">
            ...
            <div style="display:flex; gap:12px; margin-top:1.5rem;">
              <a href="${approveUrl}" style="padding:12px 24px; background:#10b981; color:#fff; border-radius:8px; text-decoration:none; font-weight:600; display:inline-block;">✓ Approve</a>
              <a href="${rejectUrl}" style="padding:12px 24px; background:#dc2626; color:#fff; border-radius:8px; text-decoration:none; font-weight:600; display:inline-block;">✗ Reject</a>
            </div>
