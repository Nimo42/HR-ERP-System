...
export async function GET(request) {
  try {
    ...
    if (role === 'Admin') {
      ...
      const hrManagerCount = await prisma.user.count({ where: { role: 'HR Manager', deletedAt: null } });
      ...
      stats.hrManagerCount = hrManagerCount;
      ...
      stats.leaveBalances = balances.map(b => ({
        ...
        balance: b.balance,
        quota: b.leaveType.quota,
        isEmergency: b.leaveType.isEmergency
      }));
